...
export class Pool {
  ...
  public getOutputAmountSync(
    ...
    const { amountCalculated: outputAmount, sqrtRatioX96, liquidity, tickCurrent } = this.swapSync(
      zeroForOne,
      inputAmount.quotient,
      sqrtPriceLimitX96
    )
  ...
  private swapSync(
    ...
    while (JSBI.notEqual(state.amountSpecifiedRemaining, ZERO) && state.sqrtPriceX96 != sqrtPriceLimitX96) {
      ...
      // by simply traversing to the next available tick, we instead need to exactly replicate
      // tickBitmap.nextInitializedTickWithinOneWord
      ;[step.tickNext, step.initialized] = this.tickDataProviderSync.nextInitializedTickWithinOneWord(
        state.tick,
        zeroForOne,
        this.tickSpacing
      )
      ...
      if (JSBI.equal(state.sqrtPriceX96, step.sqrtPriceNextX96)) {
        // if the tick is initialized, run the tick transition
        if (step.initialized) {
          let liquidityNet = JSBI.BigInt(this.tickDataProvider.getTickSync(step.tickNext).liquidityNet)
          // if we're moving leftward, we interpret liquidityNet as the opposite sign
          // safe because liquidityNet cannot be type(int128).min
          if (zeroForOne) liquidityNet = JSBI.multiply(liquidityNet, NEGATIVE_ONE)
